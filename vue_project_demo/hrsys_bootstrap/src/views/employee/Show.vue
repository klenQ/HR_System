<template>
  <div id="container">
        ...
        <div class="col-sm-2 ">
          <select class="form-control" v-model="form['dep.id']">
            ...
            <option v-for="dep in depList"
                    v-text="dep.name" v-bind:key="dep.id" value="dep.id">
            </option>
          </select>
        </div>
        <div class="col-sm-2">
    ...
    <table class="table table-striped table-bordered table-hover">
      ...
      <tbody>
      <tr class="data" v-for="emp in list" v-bind:key="emp.id" v-bind:class="{selected:emp.id==selectedId}" @click="selectTr(emp.id)">
...
export default {
  ...
  methods: {
    search: function () {
      this.axios.get('search', {params: this.form})
          .then(function (res) {
            this.list = res.data;
          }.bind(this))
    },
    searchDep: function () {
      this.axios.get('/dep/search')
          .then(function (res) {
    ...
    showUpdate: function () {
      if (this.selectedId > -1) {
        this.$router.push({ name: "EmpAdd", params:{id:this.selectedId} })
    ...
    deleteData: function () {
      if (this.selectedId > -1) {
        this.axios.get('delete', {params: {id: this.selectedId}})
            .then(function (res) {
              if(res.date ==true){
                this.search();
              }else{

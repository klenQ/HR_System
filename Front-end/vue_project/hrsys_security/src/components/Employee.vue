...
export default {
  ...
  data() {
    return {
      uploadAddr: this.tools.serverAddr + 'emp/upload',
      imageUrl: "",
  ...
  methods: {
    ...
    searchDep: function () {
      this.axios.get(this.tools.serverAddr + 'dep')
          .then(function (res) {
            this.depList = res.data;
          }.bind(this))
    },
    searchById: function () {
      this.axios.get(this.tools.serverAddr + 'emp/' + this.selectedId)
          .then(function (res) {
            this.form = res.data;
    ...
    add: function () {
      this.axios.post(this.tools.serverAddr + 'emp', this.form)
          .then(function (res) {
            if (res.data == true) {
              this.$message({
                message: '保存成功',
    ...
    deleteData: function () {
      if (this.selectedId > -1) {
        this.axios.delete(this.tools.serverAddr + 'emp/' + this.selectedId)
            .then(function (res) {
              if (res.data == true) {
                this.search();
              } else {
